...
const BookingFlowScreen = ({ navigation }) => {
  ...
  const toggleService = (serviceId) => {
    setSelectedServices((prev) => {
      ...
      if (selectedComboId) {
        const selectedCombo = serviceCombos.find(
          (combo) => combo.packageId === selectedComboId
        );
        if (selectedCombo) {
          const comboServices = selectedCombo.services;
          const hasAllComboServices = comboServices.every((service) =>
  ...
  const selectCombo = (combo) => {
    ...
    if (selectedComboId === combo.packageId) {
      setSelectedComboId(null);
      setSelectedServices([]);
    } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ProgressBar currentStep={1} onBackPress={() => navigation.goBack()} />
        ...
        <View style={styles.comboSection}>
          {serviceCombos.map((combo) => (
            <TouchableOpacity
              ...
              style={[
                styles.comboCard,
                isComboSelected(combo) && styles.comboCardSelected,
              ]}
                  ...
                  .sort((a, b) => {
                    ...
                    const aInCombo =
                      ...
                      serviceCombos
                        ...
                        ?.services.includes(a.packageId);
                    const bInCombo =
                      selectedComboId &&
                      serviceCombos
                        .find((combo) => combo.packageId === selectedComboId)
                        ?.services.includes(b.packageId);

                    if (aInCombo && !bInCombo) return -1;
                    if (!aInCombo && bInCombo) return 1;

                    // Trong cùng nhóm, sắp xếp theo trạng thái chọn
                    const aSelected = isServiceSelected(a.packageId);
                    const bSelected = isServiceSelected(b.packageId);
